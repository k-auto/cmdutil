...
def decrypt_file(input_file, output_file, key):
    ...
    try:
        ...
        with open(temp_path, 'rb') as tf, open(input_file, 'rb') as fin:
...
def createkp(folder, key=None, location=None):
    ...
    if location is None:
        location = Path.cwd()
    kpbuild_path = folder_path / "kpbuild.py"
    if not kpbuild_path.exists():
        with open(kpbuild_path, "w") as f:
            f.write("""# KPBuild

# "kpbuild.py" identifies a folder as a Knexyce Package (KP) and is auto-generated if missing.
...
def openkp(package, key=None, location=None):
    ...
    if location:
        location = (Path(location) / package_name).resolve()
    else:
    ...
    kpbuild_path = location / "kpbuild.py"
    subprocess.run([os.sys.executable, str(kpbuild_path)], cwd=str(location), check=True)
